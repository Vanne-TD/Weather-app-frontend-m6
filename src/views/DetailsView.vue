...
<template>
  <main class="container py-4">
    ...
    <button class="btn btn-outline-primary mb-3" @click="volver">
      ← Volver
    </button>
    ...
    <!-- Clima actual -->
    <!-- <div v-if="weather" class="card p-3 mb-4 text-center">
      <h2>{{ weather.weather[0].description }}</h2>
      <img
        :src="`https://openweathermap.org/img/wn/${weather.weather[0].icon}@2x.png`"
        alt="icono"
      />
      <p class="display-4">{{ Math.round(weather.main.temp) }}°</p>

      <p>Humedad: {{ weather.main.humidity }}%</p>
      <p>Viento: {{ weather.wind.speed }} km/h</p>

      <button class="btn btn-secondary mt-3" @click="toggleUnits">
        Cambiar a {{ units === 'metric' ? '°F' : '°C' }}
      </button>
    </div> -->
